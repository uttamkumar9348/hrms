...
//    }
    ...
    public function employeeTotalApprovedLeavesForGivenLeaveType($leaveType, $year)
    {
    ...
    public function getEmployeeLatestLeaveRequestBetweenFromAndToDate($select = ['*'], $data)
    {
        return LeaveRequestMaster::query()
            ->select($select)
            ->where(function ($query) use ($data) {
                $query->whereBetween('leave_from', [$data['from_date'], $data['to_date']])
                    ->orWhereBetween('leave_to', [$data['from_date'], $data['to_date']]);
            })
