                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select" name="block_id" id="block_id" placeholder="Select Block" >
                                        <option value="">{{__('Select Block')}}</option>
                                        @foreach(App\Models\Block::all() as $block)
                                        <option {{@request()->block_id && request()->block_id == $block->id ? 'selected' : '' }} value="{{$block->id}}">{{$block->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select" name="gram_panchyat_id" id="gram_panchyat_id" placeholder="Select Gram Panchyat" >
                                        <option value="">{{__('Select Gram Panchyat')}}</option>
                                        @foreach(App\Models\GramPanchyat::all() as $gp)
                                        <option {{@request()->gram_panchyat_id && request()->gram_panchyat_id == $gp->id ? 'selected' : '' }} value="{{$gp->id}}">{{$gp->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select" name="village_id" id="village_id" placeholder="Select Village">
                                        <option value="">{{__('Select Village')}}</option>
                                        @foreach(App\Models\Village::all() as $village)
                                        <option {{@request()->village_id && request()->village_id == $village->id ? 'selected' : '' }} value="{{$village->id}}">{{$village->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select" name="zone_id" id="zone_id"  placeholder="Select Country">
                                        <option value="">{{__('Select Zone')}}</option>
                                        @foreach(App\Models\Zone::all() as $zone)
                                        <option {{@request()->zone_id && request()->zone_id == $zone->id ? 'selected' : '' }} value="{{$zone->id}}">{{$zone->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select" name="center_id" id="center_id" placeholder="Select Center">
                                        <option value="">{{__('Select Center')}}</option>
                                        @foreach(App\Models\Center::all() as $center)
                                        <option {{@request()->center_id && request()->center_id == $center->id ? 'selected' : '' }} value="{{$center->id}}">{{$center->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group col-md-3">
                                ...
                                {{ Form::date('date_of_harvesting_from', @request()->date_of_harvesting_from ? \Carbon\Carbon::parse(request()->date_of_harvesting_from)->format('Y-m-d') : ''  , array('class' => 'form-control','id' => 'date_of_harvesting_from')) }}
                            </div>
                            <div class="form-group col-md-3">
                                {{ Form::label('date_of_harvesting', __('Date of Harvesting To'),['class'=>'form-label']) }}
                                {{ Form::date('date_of_harvesting_to', @request()->date_of_harvesting_to ? \Carbon\Carbon::parse(request()->date_of_harvesting_to)->format('Y-m-d') : '' , array('class' => 'form-control','id' => 'date_of_harvesting_to')) }}
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    {{ Form::label('seed_category_id', __('Seed Category'),['class'=>'form-label']) }}
                                    <select class="form-control select" name="seed_category_id" id="seed_category_id" placeholder="Select Seed Category">
                                        <option value="">{{__('Select Seed Category')}}</option>
                                        @foreach(App\Models\SeedCategory::all() as $seed_category)
                                            <option {{@request()->seed_category_id && request()->seed_category_id == $seed_category->id ? 'selected' : '' }} value="{{ $seed_category->id }}">{{ $seed_category->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group col-md-3">
                                ...
                                <select class="form-control select" name="type" id="type" placeholder="Select Type">
                                    ...
                                    <option {{@request()->type && request()->type == 'Plant' ? 'selected' : '' }}value="Plant">Plant</option>
                                    <option {{@request()->type && request()->type == 'R-1' ? 'selected' : '' }}value="R-1">R-1</option>
                                    <option {{@request()->type && request()->type == 'R-2' ? 'selected' : '' }}value="R-2">R-2</option>
                                    <option {{@request()->type && request()->type == 'R-3' ? 'selected' : '' }}value="R-3">R-3</option>
                                    <option {{@request()->type && request()->type == 'R-4' ? 'selected' : '' }}value="R-4">R-4</option>
                                    <option {{@request()->type && request()->type == 'R-5' ? 'selected' : '' }}value="R-5">R-5</option>
                                </select>
                            </div>
                        </div>
                        <div class="row">                            
                    ...
                    <div class="table-responsive">
                        <table class="table dataTable">
                            ...
                            @foreach ($farming_details as $farming_detail)
                                <tr class="font-style">
                                    <td><input type="checkbox" class="farming-detail" name="farming_detail[{{$farming_detail->id}}]" id="farming-detail" value="0"></td>
