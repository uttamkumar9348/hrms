            ...
            <tr>
                ...
                <td>
                    <table class="no-space" style="width: 45%;margin-left: auto;">
                        ...
                        <tr>
                            <td>{{__('Number')}}:</td>
                            <td class="text-right">{{\App\Models\Utility::purchaseNumberFormat($settings,$purchase->purchase_id)}}</td>
                        </tr>
                        <tr>
                            <td>{{__('Purchase Date')}}:</td>
                            <td class="text-right">{{\App\Models\Utility::dateFormat($settings,$purchase->purchase_date)}}</td>
                        </tr>
                        ...
                        <tr>
                            <td colspan="2">
                                <div class="view-qrcode">
                                    {!! DNS2D::getBarcodeHTML(route('admin.purchase.link.copy',\Crypt::encrypt($purchase->purchase_id)), "QRCODE",2,2) !!}
                                </div>
                            </td>
                        </tr>
                        </tbody>
    ...
    <div class="purchase-body">
        ...
        <table class="add-border purchase-summary" style="margin-top: 30px;">
            ...
            @if(isset($purchase->itemData) && count($purchase->itemData) > 0)
                @foreach($purchase->itemData as $key => $item)
                    <tr>
                        ...
                        <td>{{\App\Models\Utility::priceFormat($settings,$item->price * $item->quantity -  $item->discount + $itemtax)}}</td>
            ...
            <tr>
                ...
                <td>{{\App\Models\Utility::priceFormat($settings,$purchase->totalRate)}}</td>
                <td>{{\App\Models\Utility::priceFormat($settings,$purchase->totalDiscount)}}</td>
                <td>{{\App\Models\Utility::priceFormat($settings,$purchase->totalTaxPrice) }}</td>
                <td>{{\App\Models\Utility::priceFormat($settings,$purchase->getSubTotal())}}</td>
            </tr>
            <tr>
                <td colspan="4"></td>
                <td colspan="2" class="sub-total">
                    <table class="total-table">
                        ...
                        @if($purchase->getTotalDiscount())
                            <tr>
                                <td>{{__('Discount')}}:</td>
                                <td>{{\App\Models\Utility::priceFormat($settings,$purchase->getTotalDiscount())}}</td>
                            </tr>
                        @endif
                        ...
                        <tr>
                            <td>{{__('Total')}}:</td>
                            <td>{{\App\Models\Utility::priceFormat($settings,$purchase->getSubTotal()-$purchase->getTotalDiscount()+$purchase->getTotalTax())}}</td>
                        </tr>
                        <tr>
                            <td>{{__('Paid')}}:</td>
                            <td>{{\App\Models\Utility::priceFormat($settings,($purchase->getTotal()-$purchase->getDue()))}}</td>
                        </tr>

                        <tr>
                            <td>{{__('Due Amount')}}:</td>
                            <td>{{\App\Models\Utility::priceFormat($settings,$purchase->getDue())}}</td>
                        </tr>
